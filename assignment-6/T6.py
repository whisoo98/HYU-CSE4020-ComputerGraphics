...
def render():
    ...
    t = glfw.get_time()
    xang = t*f12
    yang = np.radians(30)*f12
    zang = np.radians(30)*f12
...
def main():
    ...
    if not glfw.init():
        return
    window = glfw.create_window(480,480,'2018008240-6-1', None,None)
    if not window:
        glfw.terminate()
        return
