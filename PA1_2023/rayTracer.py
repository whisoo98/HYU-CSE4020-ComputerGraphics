...
def main():
    ...
    global ambient
    amb = 0/255
    ambient = np.array([amb, amb, amb]).astype(np.float64)
    BLACK = np.array([0., 0., 0.]).astype(np.float64)
    ...
    for c in root.findall('camera'):
        ...
        if (c.findtext('projDistance')):
            projDistance = np.array(c.findtext(
                'projDistance').split()).astype(np.float64)
        viewWidth = np.array(c.findtext('viewWidth').split()).astype(np.float64)
        viewHeight = np.array(c.findtext(
            'viewHeight').split()).astype(np.float64)

    view = View(viewPoint, viewDir, viewUp, projNormal, viewWidth, viewHeight,
                projDistance, intensity)
    ...
    for c in root.findall('shader'):
        ...
        if _type == 'Phong':
            ...
            exponent = np.array(c.findtext(
                'exponent').split()).astype(np.float64)
            shader = ShaderPhong(diffuse=diffuseColor_c, specular=specularColor,
                                 exponent=exponent, type=_type)
        else:
            diffuseColor_c = np.array(c.findtext(
                'diffuseColor').split()).astype(np.float64)
...
def mapPixelToImage(view, imgSize, surfaces, lights):
    ...
    normal_img_plane = view.projNormal       # w

    w_img_plane = normal_img_plane                        # w
    u_img_plane = np.cross(view.viewUp, normal_img_plane)  # u
    v_img_plane = np.cross(u_img_plane, normal_img_plane)  # v
    ...
    for i in np.arange(imgSize[1]):
        for j in np.arange(imgSize[0]):
            u = l + width*j/pixel_x
            v = b + height*i/pixel_y
            pixel_on_img_plane = view_point + u*u_unit + \
                v*v_unit - dist_img_plane*w_unit  # s
            direction = pixel_on_img_plane-view_point
            t, surface_idx, blocked = doRayTracing(
                view_point, direction, surfaces)  # rayTracing
...
def shadow(start_point, direction_point, t, surface_idx, lights, surfaces, view):
    result = np.array([0, 0, 0]).astype(np.float64)
    ...
    for light in lights:
        ...
        check_t, check_idx, blocked = doRayTracing(
            shadRay_start_point, direction, surfaces)
...
def doShading(surface_point, view_point, light, surface):
    ...
    if surface.shader.__class__.__name__ == "ShaderPhong":
        result = shadePhong(surface_to_light, surface_to_view,
                            surface_normal, surface, light)

    elif surface.shader.__class__.__name__ == "ShaderLambertian":
        result = shadeLambertian(
            surface_to_light, surface_normal, surface, light)
...
def shadeLambertian(to_light, normal, surface, light):
    result = np.array([0, 0, 0]).astype(np.float64)
    result[0] = result[0] + surface.shader.diffuse[0] * \
        light.intensity[0] * max(0, np.dot(to_light, normal))
    result[1] = result[1] + surface.shader.diffuse[1] * \
        light.intensity[1] * max(0, np.dot(to_light, normal))
    result[2] = result[2] + surface.shader.diffuse[2] * \
        light.intensity[2] * max(0, np.dot(to_light, normal))
    return result
    # return color
